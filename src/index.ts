...
export interface MarkdownItCodeGroupOptions {
  /**
...
function getIcon(type: string) {
  ...
  if (type.includes('yarn')) {
    return {
      type: 'yarn',
      icon: DEFAULT_GITHUB_ICONS.yarn,
    }
  }
  if (type.includes('bun')) {
    return {
      type: 'bun',
      icon: DEFAULT_GITHUB_ICONS.bun,
    }
  }
}

const MarkdownItCodeGroup: MarkdownIt.PluginWithOptions<MarkdownItCodeGroupOptions> = (md, options = {}) => {
  const {
      ...
      if (tokens[i].content === '::code-group-open' && tokens[i].type === 'inline') {
        ...
        while (tokens[i]?.content !== '::code-group-close' && i <= tokens.length) {
          ...
          if (tokens[i]?.tag === 'code') {
            const { type } = getIcon(tokens[i].info) as any
            if (type === 'npm')
              tokens[i].attrs = [['class', `pkg-${type} active`]]
            else
              tokens[i].attrs = [['class', `pkg-${type}`]]
          }
        }
        ...
        const tabs = codeGroupContent.map((v) => {
          const { icon, type } = getIcon(v.info) as any
          if (type === 'npm') {
            return `<div class="markdown-group-tab-item active" data-pkg="${type}">${icon}
            <span>${type}</span>
            </div>`
          }
          else {
            return `<div class="markdown-group-tab-item" data-pkg="${type}">${icon}
            <span>${type}</span>
            </div>`
          }
        }).join('')
...
export default MarkdownItCodeGroup
